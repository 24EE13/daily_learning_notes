...
它们的判断依据不是字符串长度而是ASCII码，从下标为0的字符开始一一比对，那个ASCII码值高那个就更“大”。

诸如`s1 > s2`、`s1 == s2`之类的运算是通过运算符重载来实现的，本质上是函数的一种实现形式。

## string 与其他类型转换

### 转换成整型

`int stoi(const string& str,sizeof* idx = 0,int base = 10)`   
`long int stol(const string& str,sizeof* idx = 0,int base = 10)`   
...
`double stod(const string& str,sizeof* idx = 0,int base = 10)`   
`float stof(const string& str,sizeof* idx = 0,int base = 10)`   
